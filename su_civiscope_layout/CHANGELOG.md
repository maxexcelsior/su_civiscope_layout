...
## [0.1.9-beta] - 2026-07-30

### 修复 🐛

- **塔楼叠放建筑真高计算修正** (`logic/bldg_manager.rb`)

  - 多塔楼退台式叠放时，上方塔楼的建筑真高现在正确累计下方所有CIM体块（包括塔楼）的高度
  - `find_podiums_under_tower` 重构为 `find_buildings_under_entity`，移除类型过滤限制
  - 示例：裙楼A(12m)+裙楼B(10m)+塔楼C(40m)+塔楼D(30m) → 选中塔楼D时建筑真高=12+10+40+30=92m

- **叠放体块避难层分配** (`logic/bldg_manager.rb`)

  - 新增 `compute_block_refuge_floors` 方法，新增 `find_top_of_stack` 向上遍历堆叠顶部的体块获取建筑总高，按体块的堆叠区间分配避难层
  - 上方塔楼的避难层不再全部扣减在自身，而是按50m/100m的物理位置分配到对应的体块上
  - 示例：裙楼A(12m)+裙楼B(10m)+塔楼C(40m)+塔楼D(50m) → 塔楼D真高=112m
    - 避难层位置：50m, 100m
    - 塔楼C(22~62m)含50m → 分配1层，塔楼D(62~112m)含100m → 分配1层
  - 修复热重载后旧存储值不刷新的问题：`render_targets` 改为按最新计算值显示

- **非堆叠顶部体块不再生成屋顶构筑物** (`logic/bldg_manager.rb`)

  - 中间层塔楼即使建筑真高＞100m也不会生成屋顶构筑物
  - 使用 `find_top_of_stack` 判断当前体块是否为堆叠的最顶层体块，仅最顶层体块生成屋顶构筑物
### 2026-07-27


## [0.1.8-beta] - 2026-07-27

### 新增功能 ✨

- **地下空间折减系数** (`settings.rb`, `ui/ui_settings.html`)

  - 统计配置标签页新增"地下空间—折减系数"输入项，默认 95%
  - 支持百分比输入（0-100%），配置持久化到 settings.json
  - 地下空间采用固定折减系数，不按高度区间折减

- **地上/地下计容面积折减前后区分** (`ui/stats_dialog.rb`, `ui/ui_stats.html`)

  - 单选 CIM 地块建筑列表底部"地上/地下总计容建筑面积"新增折减后数值列
  - 折减开启时显示黄色折减后数值，关闭时仅显示原始值
  - 多选地块/普通 Group 汇总的"总计容建筑面积（折减后）"下方新增"地上/地下总计容建筑面积（折减后）"子项

### 优化改进 🔧

- **折减系数引擎扩展** (`settings.rb`)

  - `compute_reduction_factor` 新增 `'地下空间'` 类型支持，返回 unified 折减系数
  - `save_reduction_settings` 新增 `underground_coeff` 参数

- **地下空间自动切换图层** (`utils/attr_helper.rb`, `logic/bldg_manager.rb`, `ui/picker_tool.rb`)

  - 建筑类型设为"地下空间"时，自动将组件图层切换为 `CIM-mass-ug`
  - 切换回其他类型时自动恢复为 `CIM-mass`
  - 新增 `update_bldg_layer` 辅助方法，属性刷修改类型时同样生效

- **塔楼/独立折减系数调整** (`settings.rb`)

  - [0,100] 区间从 0.95→0.90，(450,500] 折减系数改为 0.75
  - 中间 7 档从 0.88125 到 0.76875 等差过渡，每档递减 1.875pp
...
- **建筑密度双模式（手动/自动）** (`settings.rb`, `logic/stats_engine.rb`, `ui/stats_dialog.rb`, `ui/ui_stats.html`)

  - 手动模式（原有）：根据 CIM 地块中 `Civiscope_建筑基底` 材质面积统计
  - 自动模式（新增）：根据地块内紧贴地面的 CIM 建筑标准层面积（`base_area`）之和汇总，自动排除叠放在裙楼上的塔楼
  - 统计中心用地标签"建筑密度"旁新增模式下拉选择器
  - 选择自动/手动模式后自动刷新并持久化到 settings.json
  - 所有视图（BIM、BP组只读、多选汇总）均支持双模式切换
